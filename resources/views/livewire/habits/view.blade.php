    ...
    $this->lastSevenDays = $this->habit
        ...
        ->map(function ($sessions) {
            return $sessions->sum(function ($session) {
                ...
                return $session->start_time->diffInMinutes($endTime);
            });
        });
...
$maxSession = computed(function () {
    return $this->maxMinutesPerDay > 0
        ? CarbonInterval::minutes($this->maxMinutesPerDay)
            ->cascade()
            ->format('%H:%I')
        : '00:00';
});
...
<div>
    ...
    <div class="mb-6">
        ...
        <p>Longest session this week: {{ floor($maxMinutesPerDay / 60) }}h {{ $maxMinutesPerDay % 60 }}m</p>
